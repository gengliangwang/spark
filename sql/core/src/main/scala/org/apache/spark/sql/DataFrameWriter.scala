...
final class DataFrameWriter[T] private[sql](ds: Dataset[T]) {
  ...
  def save(): Unit = {
    ...
    extraOptions += (DataSourceUtils.SCHEMA_KEY -> df.schema.toDDL)
    ...
    val table = if (!shouldUseV1Source) {
  ...
  private def loadTableFromPathCatalog(cls: Class[_]): Option[Table] = {
    ...
    val shouldWrite = modeForDSV2 match {
      case SaveMode.Ignore => catalog.tableExists(ident)
      case _ => true
    }
    if (!shouldWrite) {
      return None
