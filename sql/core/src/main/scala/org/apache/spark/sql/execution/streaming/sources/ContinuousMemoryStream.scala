...
class ContinuousMemoryStream[A : Encoder](id: Int, sqlContext: SQLContext, numPartitions: Int = 2)
  ...
  override def getSplitManager(meta: Metadata): SplitManager = new SplitManager {
    override def getSplits: Array[InputSplit] = ContinuousMemoryStream.this.synchronized {
      ...
      startOffset.partitionNums.map {
        case (part, index) => ContinuousMemoryStreamInputSplit(endpointName, part, index)
      }.toArray
    }
  }
  ...
  override def getReaderProvider(meta: Metadata): SplitReaderProvider = new SplitReaderProvider {
    override def createRowReader(split: InputSplit): InputPartitionReader[InternalRow] = {
      val s = split.asInstanceOf[ContinuousMemoryStreamInputSplit]
      new ContinuousMemoryStreamInputPartitionReader(
        s.driverEndpointName, s.partition, s.startOffset)
    }
  }
...
case class ContinuousMemoryStreamInputSplit(
    driverEndpointName: String,
    partition: Int,
    startOffset: Int) extends InputSplit
