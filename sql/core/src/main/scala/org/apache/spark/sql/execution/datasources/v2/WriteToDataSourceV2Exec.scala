...
object DataWritingSparkTask extends Logging {
  def run(
    ...
    val dataWriter = writerFactory match {
      case f: FileWriterFactory if !iter.hasNext && partId != 0 => f.createEmptyWriter(partId)
      case other => other.createWriter(partId, taskId)
    }
