...
object PushVariantIntoScan extends Rule[LogicalPlan] {
  override def apply(plan: LogicalPlan): LogicalPlan = plan match {
    ...
    case _ => plan.transformDown {
      ...
      // Note: DataSource V2 variant pushdown is handled via SupportsPushDownVariantProjections
      // which extends ScanBuilder. The pushdown happens during scan building phase in
      // V2ScanRelationPushDown, not in this rule.
    }
  }
