...
import org.apache.spark.sql.execution.datasources.orc.{Contact, OrcFileFormat, Person}
...
abstract class OrcQueryTest extends OrcTest {
  ...
  test("Read/write binary data") {
    withOrcFile(BinaryData("test".getBytes(StandardCharsets.UTF_8)) :: Nil) { file =>
      val bytes = loadOrcFile(file).head().getAs[Array[Byte]](0)
  ...
  test("SPARK-9170: Don't implicitly lowercase of user-provided columns") {
    withTempPath { dir =>
      ...
      loadOrcFile(path).schema("Acol")
      intercept[IllegalArgumentException] {
        loadOrcFile(path).schema("acol")
      }
      checkAnswer(loadOrcFile(path).select("acol").sort("acol"),
  ...
  test("SPARK-15198 Support for pushing down filters for boolean types") {
    withSQLConf(SQLConf.ORC_FILTER_PUSHDOWN_ENABLED.key -> "true") {
      ...
      withOrcFile(data) { file =>
        val df = loadOrcFile(file).where("_2 == true")
  ...
  test("Support for pushing down filters for decimal types") {
    withSQLConf(SQLConf.ORC_FILTER_PUSHDOWN_ENABLED.key -> "true") {
      ...
      withTempPath { file =>
        ...
        val df = loadOrcFile(file.getCanonicalPath).where("a == 2")
        val actual = stripSparkFilter(df).count()
  ...
  test("Support for pushing down filters for timestamp types") {
    withSQLConf(SQLConf.ORC_FILTER_PUSHDOWN_ENABLED.key -> "true") {
      ...
      withTempPath { file =>
        ...
        spark.createDataFrame(data).toDF("a").repartition(10)
          .write.orc(file.getCanonicalPath)
        val df = loadOrcFile(file.getCanonicalPath).where(s"a == '$timeString'")
        val actual = stripSparkFilter(df).count()
  ...
  ignore("read from multiple orc input paths") {
    ...
    val df = loadOrcFile(path1.getCanonicalPath, path2.getCanonicalPath)
    assert(df.count() == 20)
  }
...
class OrcQuerySuite extends OrcQueryTest with SharedSQLContext {
  ...
  test("SPARK-21791 ORC should support column names with dot") {
    withTempDir { dir =>
      ...
      assert(loadOrcFile(path).collect().length == 2)
    }
  }
