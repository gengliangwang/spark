...
abstract class OrcTest extends QueryTest with SQLTestUtils with BeforeAndAfterAll {
  ...
  val orcFormat: String = "org.apache.spark.sql.execution.datasources.v2.orc.OrcDataSource"
  ...
  protected def withOrcDataFrame[T <: Product: ClassTag: TypeTag]
    ...
    withOrcFile(data)(path => f(loadOrcFile(path)))
  }

  protected def loadOrcFile(path: String): DataFrame = {
