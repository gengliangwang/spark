...
class KryoSerializerSuite extends KVStoreSerializerSuite {
  override protected def serializer: KVStoreSerializer = new KVKryoSerializer()
}

abstract class KVStoreSerializerSuite extends SparkFunSuite {
  protected def serializer: KVStoreSerializer
  ...
  test("Executor Summary") {
    ...
    val resources =
      Map("resource1" -> new ResourceInformation("re1", Array("add1", "add2")),
        "resource2" -> new ResourceInformation(null, null))
